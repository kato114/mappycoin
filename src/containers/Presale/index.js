...
const Presale = () => {
  ...
  const [timerRefreshCount, serTimerRefreshCount] = useState(0)
  ...
  useEffect(() => {
    const contractFetchTimer = setTimeout(() => {
      setRefreshCount(refreshCount + 1)
    }, 1000)

    const timerRefreshTimer = setTimeout(() => {
      serTimerRefreshCount(timerRefreshCount + 1)
    }, 60000)
    ...
    return () => {
      alert.removeAll()
      clearTimeout(contractFetchTimer)
      clearTimeout(timerRefreshTimer)
    }
  ...
  }, [startTimeLeft, timerRefreshCount])
  ...
  return (
    <PresaleComponent
      ...
      token={getAsset("MAPPY")}
      tokenContract={getContract("MAPPY")}
      presaleContract={getContract("PRESALE")}
      presaleSupply={12000000}
      rate={
